...
sC = math.max(sY / 1080, 0.66)

-- THESE CAN BE CHANGED
triggerKey = "tab" -- default button to open/close scoreboard
drawOverGUI = false -- draw scoreboard over gui?
...
contentFont = dxCreateFont(":dx_fonts/default-bold.ttf", sC * 10, true)
columnFont = dxCreateFont(":dx_fonts/regular2.ttf", sC * 9.3, false)
serverInfoFont = dxCreateFont(":dx_fonts/regular2.ttf", sC * 10, false)
...
addEventHandler( "onClientResourceStart", getResourceRootElement( getThisResource() ),
	function ( resource )
		...
		scoreboardIsToggleable = defaultSettings.toggleable
		showServerInfo = defaultSettings.showserverinfo
		showGamemodeInfo = defaultSettings.showgamemodeinfo
		showTeams = defaultSettings.showteams
		useColors = defaultSettings.usecolors
		drawSpeed = defaultSettings.drawspeed
		scoreboardScale = defaultSettings.scale
		teamHeaderFont = defaultSettings.teamfont
...
function toggleScoreboard( _, state )
	if getElementData(localPlayer, 'player.logged') then
		...
		triggerEvent('onClientDXScoreboardToggle', localPlayer, state)
	end
end
	...
	if #scoreboardColumns ~= 0 then
			...
			for key, player in ipairs( players ) do
				if not getPlayerTeam( player ) or not (showTeams or (serverInfo.forceshowteams and not serverInfo.forcehideteams)) or serverInfo.forcehideteams then
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						elseif column.name == "player.zone" then
							content = getZoneName(getElementPosition( player ))
						elseif column.name == "player.money" then
							content = string.moneyFormating(getPlayerMoney( player ), '.')
						else
		...
		if scoreboardDrawn then
			...
			if serverInfo.server and showServerInfo then
				dxDrawText( "" .. serverInfo.server, topX+s(5), y, topX+scoreboardDimensions.width-s(10), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), cServerInfo, fontscale(serverInfoFont, s(0.75)), serverInfoFont, "left", "top", false, false, drawOverGUI )
			end
			...
			for key, column in ipairs( scoreboardColumns ) do
				if x ~= s(10) then
					...
					height = height+s(3)
					--dxDrawLine( topX+x-s(5), y+s(1), topX+x-s(5), y+scoreboardDimensions.height-height-s(7), cBorder, s(1), drawOverGUI )
				end
				if sortBy.what == column.name then
					local _, _, _, a = fromcolor( cHeader )
					dxDrawText( column.friendlyName or "-", topX+x+s(1+9), y+s(1), 	topX+x+s(1+(column.width * sC)), 	y+s(1)+dxGetFontHeight( fontscale(columnFont, scoreboardScale), columnFont ), tocolor( 0, 0, 0, a ), fontscale(columnFont, s(1)), columnFont, "left", "top", true, false, drawOverGUI )
					dxDrawText( column.friendlyName or "-", topX+x+s(9), 	y, 	topX+x+s((column.width * sC)), 	y+dxGetFontHeight( fontscale(columnFont, scoreboardScale), columnFont ), cHeader, fontscale(columnFont, s(1)), columnFont, "left", "top", true, false, drawOverGUI )
					...
					local _, _, _, a = fromcolor( cHeader )
					dxDrawText( column.friendlyName or "-", topX+x+s(1), 	y+s(1), 	topX+x+s(1+(column.width * sC)), 	y+s(1)+dxGetFontHeight( fontscale(columnFont, scoreboardScale), columnFont ), tocolor( 0, 0, 0, a ), fontscale(columnFont, s(1)), columnFont, "left", "top", true, false, drawOverGUI )
					dxDrawText( column.friendlyName or "-", topX+x, 	y, 	topX+x+s((column.width * sC)), 	y+dxGetFontHeight( fontscale(columnFont, scoreboardScale), columnFont ), cHeader, fontscale(columnFont, s(1)), columnFont, "left", "top", true, false, drawOverGUI )
				end
				x = x + s((column.width * sC) + 10)
			end
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if content then
							if serverInfo.allowcolorcodes and type( content ) == "table" and column.name == "name" then
								...
								for k, v in ipairs( colorCodes ) do
									...
									if firstCodePos ~= 1 and k == 1 then
										...
										dxDrawText( partOfName, xPos+s(1), 	y+s(1), 	topX+x+s(1+(column.width * sC)), 	y+s(11)+dxGetFontHeight( fontscale(teamHeaderFont, scoreboardScale), teamHeaderFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(teamHeaderFont, s(1)), teamHeaderFont, "left", "top", true, false, drawOverGUI )
										dxDrawText( partOfName, xPos, 			y, 			topX+x+s((column.width * sC)), 	y+dxGetFontHeight( fontscale(teamHeaderFont, scoreboardScale), teamHeaderFont ), 	tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(teamHeaderFont, s(1)), teamHeaderFont, "left", "top", true, false, drawOverGUI )
							...
							elseif type( content ) == "table" and column.name ~= "name" then
								...
								dxDrawText( content, topX+theX+s(1), 	y+s(1), 	topX+x+s(1+(column.width * sC)), 	y+s(11)+dxGetFontHeight( fontscale(teamHeaderFont, scoreboardScale), teamHeaderFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(teamHeaderFont, s(1)), teamHeaderFont, "left", "top", true, false, drawOverGUI )
								dxDrawText( content, topX+theX, 		y, 		topX+x+s((column.width * sC)), 	y+dxGetFontHeight( fontscale(teamHeaderFont, scoreboardScale), teamHeaderFont ), 			tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(teamHeaderFont, s(1)), teamHeaderFont, "left", "top", true, false, drawOverGUI )
							end
						end
						x = x + s((column.width * sC) + 10)
					end
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if content then
							if serverInfo.allowcolorcodes and type( content ) == "table" and column.name == "name" then
								...
								for k, v in ipairs( colorCodes ) do
									...
									if firstCodePos ~= 1 and k == 1 then
										...
										local partOfName = string.sub( playerName, firstPos, secondPos )
										local textLength = dxGetTextWidth( partOfName, fontscale(contentFont, s(1)), contentFont )
										dxDrawText( partOfName, xPos+s(1), 	y+s(1), topX+x+s(1+(column.width * sC)), 	y+s(11)+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
										dxDrawText( partOfName, xPos, 		y, 		topX+x+s((column.width * sC)), 	y+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 			tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
										xPos = xPos + textLength
									end
									if useColors then
										r, g, b = v[1][1], v[1][2], v[1][3]
								...
								elseif column.isImage then
									if type(content)=="table" then
										...
										dxDrawImage( topX+theX, y+s(1), (column.imageW or 17)*scoreboardScale, 	(column.imageH or 11)*scoreboardScale, content[1], 0, 0, 0, cWhite, drawOverGUI )
											dxDrawText( content[2], topX+theX+s(1)+((column.imageW or 17)*scoreboardScale)+2, y+s(1), topX+x+s(1+(column.width * sC)), 	y+s(11)+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
											dxDrawText( content[2], topX+theX+((column.imageW or 17)*scoreboardScale)+2, y, topX+x+s((column.width * sC)), 	y+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 			tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
										else
											dxDrawText( content[2], topX+theX+s(1)+(0*scoreboardScale), y+s(1), topX+x+s(1+(column.width * sC)), 	y+s(11)+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
											dxDrawText( content[2], topX+theX+(0*scoreboardScale), y, topX+x+s((column.width * sC)), 	y+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 			tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
							...
							elseif type( content ) == "table" and column.name ~= "name" then
								...
								dxDrawText( content, topX+theX+s(1), 	y+s(1), topX+x+s(1+(column.width * sC)), 	y+s(11)+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 	tocolor( 0, 0, 0, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
								dxDrawText( content, topX+theX, 		y, 		topX+x+s((column.width * sC)), 	y+dxGetFontHeight( fontscale(contentFont, scoreboardScale), contentFont ), 			tocolor( r or 255, g or 255, b or 255, a or 255 ), fontscale(contentFont, s(1)), contentFont, "left", "top", true, false, drawOverGUI )
							end
						end
						x = x + s((column.width * sC) + 10)
					end
...
function calculateWidth()
	local width = 0
	for key, value in ipairs( scoreboardColumns ) do
		width = width + s((value.width * sC) + 10)
	end
	return width + s(10)
end
...
function getMaxPerWindow()
	...
	availableHeight = availableHeight-s(5)
...
function scoreboardClickHandler( button, state, cX, cY )
	if scoreboardDrawn and button == "left" and state == "down" then
		...
		if cX >= xMin and cX <= xMax and cY >= yMin and cY <= yMax then
			...
			if (serverInfo.gamemode or serverInfo.map) and showGamemodeInfo then y = y + dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ) end
			for key, column in ipairs( scoreboardColumns ) do
				if cX >= topX+x and cX <= topX+x+s((column.width * sC)) then
